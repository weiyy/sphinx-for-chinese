...
//
// $Id: test.c 4505 2014-01-22 15:16:21Z deogar $
//
...
void test_keywords ( sphinx_client * client )
{
	...
	words = sphinx_build_keywords ( client, "谷歌身份验证器：Google Authenticator", "test1", SPH_TRUE, &nwords );
...
*/
	const char* attr_downloads = "downloads";
	sphinx_int64_t f = 114395;
	sphinx_add_filter ( client, attr_downloads, 1, &f, SPH_FALSE );
	test_query ( client, "谷歌身份验证器", "test1" );
...
*/
	...
	sphinx_set_ranking_mode(client, SPH_RANK_EXPR, "sum(lcs*user_weight)*1000+bm25+sortlevel");
	test_query ( client, "神奇的颜色", "test1" );
	sphinx_set_ranking_mode(client, SPH_RANK_EXPR, "sum(lcs*user_weight)");
	test_query ( client, "神奇的颜色", "test1" );
	sphinx_set_ranking_mode(client, SPH_RANK_EXPR, "bm25+IF(downloads > 0, IF(downloads > 110000, -2, 1000), 0)");
	//test_query ( client, "超级", "test1" );
	test_query ( client, "神奇的颜色", "test1" );
	//test_query ( client, "超级 | 猴子", "test1" );
