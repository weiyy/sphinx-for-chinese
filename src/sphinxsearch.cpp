...
int ExtRanker_T<STATE>::GetMatches ()
{
	...
	{
		//printf("m_dMatches[iMatches].m_iWeight:%u\n", m_dMatches[iMatches].m_iWeight);
		// keep ranking
...
struct RankerState_ProximityBM25Exact_fn : public ISphExtra
{
	...
	DWORD Finalize ( const CSphMatch & tMatch )
	{
		...
		//printf("total:%d, tMatch:%u, uRank:%u\n", tMatch.m_iWeight + uRank, tMatch.m_iWeight, uRank);
		return uRank;
	}
};
...
public:
	...
	void				Update ( const ExtHit_t * pHlist );
	//DWORD				Finalize ( const CSphMatch & tMatch );
	uint64_t				Finalize ( const CSphMatch & tMatch );
...
uint64_t RankerState_Expr_fn<NEED_PACKEDFACTORS, HANDLE_DUPES>::Finalize ( const CSphMatch & tMatch )
{
	// finishing touches
